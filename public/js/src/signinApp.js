...
var service="";
var serviceSplit = window.location.hash.split('service=');
if (serviceSplit.length > 1) {
    service = serviceSplit[1].split('&')[0];
};
if (service) {
    console.log(`Detected service: ${service}`);
    if (service.replace(/[^0-9-a-z.]/gi, '').endsWith(".parkplanr.app") || service=="parkplanr.app") {
	...
	localStorage.service = config('defaultService');
    };
} else {
  ...
  firebase.auth().onAuthStateChanged(async function (user) {
	...
	const idToken = await firebase.auth().currentUser.getIdToken();
    	const postIdResult = await fetch("/signin", {
		"headers": {
	...
	if (idResult.status) {
		...
		const service = localStorage.service;
		delete(localStorage.service);
...
async function resetPassword () {
  ...
  try {
	resetResult = await firebase.auth().sendPasswordResetEmail(email);
...
function signinEmail () {
  ...
  if (!email) {
    ...
    toastr.error("Please enter your email address", 'Error signing in with email address and password')
    return
  }
  if (!password) {
    console.log('No password provided')
    toastr.error("Please enter your password", 'Error signing in with email address and password')
    return
  }
  ...
  firebase
	...
	case "auth/user-not-found":
		...
		error.userMessage = "We couldn't find a user account for that email address";
		break;
	case "auth/wrong-password":
		console.log("Wrong password");
		error.userMessage = "That password isn't right, if you have forgotten it click Lost Password below";
		break;
	case "auth/invalid-email":
		console.log("Email address invalid format");
		error.userMessage = "Whoops! That email address doesn't look quite right";
		break;
	default:
      ...
      toastr.error(error.userMessage, 'Error signing in with email address and password')
    })
}
function signupEmail () {
  ...
  if (!email) {
    console.log('No email address provided')
    toastr.error("Please enter your email address", 'Error signing up with email address and password')
    return
  }
  if (!password) {
    console.log('No password provided')
    toastr.error("Please enter your password", 'Error signing up with email address and password')
    return
  }
  ...
  firebase
	...
	default:
      ...
      toastr.error(error.userMessage, 'Error signing up with email address and password')
    })
}
